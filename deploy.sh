...
VERIFY_ENV_VARS=${VERIFY_ENV_VARS:-"NODE_ENV OPENAI_CHAT_MODEL OPENAI_CHAT_REASONING_EFFORT OPENAI_CHAT_VERBOSITY CONTACT_DELIVERY_METHOD CHAT_CONTACT_EMAIL_ENABLED CONTACT_EMAIL CONTACT_EMAIL_TO EMAIL_HOST"}
...
if [ -z "$CONTACT_WEBHOOK_URL_VALUE" ] && { [ -z "$EMAIL_HOST_VALUE" ] || [ -z "$EMAIL_USER_VALUE" ] || [ -z "$EMAIL_PASS_VALUE" ]; }; then
  echo "Warning: contact delivery is not fully configured." >&2
  echo "Set CONTACT_WEBHOOK_URL, or set EMAIL_HOST, EMAIL_USER, and EMAIL_PASS for SMTP." >&2
fi
